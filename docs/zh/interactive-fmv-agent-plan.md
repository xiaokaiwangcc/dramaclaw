...
- 后端 `path_analysis.py` 与前端 `pathAnalysis.ts` 都会从初始剧情状态开始做有界运行路径模拟，按真实的变量、布尔开关、访问次数、条件顺序和效果计算可达性。
- 新增 `condition_unreachable`、`runtime_unreachable`、`variable_out_of_bounds`、`automatic_cycle` 和 `path_analysis_incomplete` 统一问题码。
- 玩家主动选择形成的回访循环合法，例如“取得钥匙后返回门口”；不经过玩家输入且状态不再推进的自动循环会作为 error 阻止试玩和导出。
- `compileStoryGroup` 在生成 Ink、进入试玩或导出前统一执行故事门禁，不再静默忽略跨组连线或严重路径问题。
- 画布试玩分为“试玩模式”和“实时生成”：试玩模式从正式起点完整试玩并沿用存档/统计；实时生成提供同屏剧情树与播放器，点击任意片段（包括尚未接入主线的孤立片段）即可用全新状态从该处开始调试，不污染正式试玩记录。
- 无视频的占位片段使用中部偏上的限宽、左对齐阅读区，长正文可滚动，占位标识置于角落；选择反馈期间暂时隐藏上一片段正文与标题，不改变反馈结束后再跳转的规则。
- 实时生成顶部“从此生成”与原“编辑结构”合并：退出播放器、定位并选中当前片段，打开片段编辑态，复用提示词编辑与单节点生成能力；不表示自动重生成后续剧情。
- 试玩模式直接恢复已有剧情存档（从保存节点的视频开头播放，不恢复视频秒数）；实时生成提供自动播放和倍速控制，重复选择同一片段会重新播放。结局文本等视频结束后展示。
- 试玩、实时生成和 HTML 导出均完整展示视频，并将选项、热点和结局限制在视频画幅内；物品锚点按实际画幅换算。剧情树支持搜索、折叠、问题筛选和双向滚动。
- 剧情树的展示入口以连线 ID 标识，根行以起点 ID 标识，与播放器的节点 ID 分离：点击同节点的不同入口只高亮所点行，自动定位也跟随该入口。播放器自行换节点时默认定位其首次展示行；引用祖先的行标为“回到此处”，其他重复引用标为“汇合到此片段”。
- 打开影游视频节点内联生成面板时，节点临时高于剧情分支标签，避免标签遮挡提示词和操作按钮；关闭或展开为独立弹窗后恢复原层级，普通视频节点不受影响。
- 剧情片段进入编辑时保持左侧视频、右侧剧情内容与制作备注的布局，生成面板只在下方展开。去掉常驻“视频已就绪／待制作”文字；保留上传中、生成中、失败、导入文件名和需检查信息。
- 互动片段单击视频框或占位区域进入编辑，点击空白或其他节点退出；键盘聚焦后 Enter/空格也可进入。媒体控件、拖动和抓手平移不触发编辑。视频上的铅笔与设起点按钮移除，“设为故事起点”收纳至片段右键菜单，仅修改当前故事组；标题上的悬浮起点标记移除，保留内部起点数据与播放规则。
- 选中故事组或组内片段时，画布顶部显示独立剧情工具栏并预留高度，画布及其坐标容器位于下方；不再叠加底部悬浮栏遮挡生成器。新增片段及编辑结构复用原画布能力。剧情连线悬停显示剪刀，断开操作保留两端节点并支持撤销。
- 条件状态组合超过分析上限时只返回分析未完成 warning，不会把未验证路径错误地判定为不可达。
- Patch 的布局与素材保留、版本冲突和局部删除测试继续作为增量编辑回归；故事组显式紧凑操作仍待实现。

### M6：视频生产与素材版本

分步实施计划与验收记录见 [M6 视频生产 MVP](interactive-video-production-mvp.md)。
...
| P1 | M5 增量编辑与质量门禁 | 条件路径模拟与统一 Ink 门禁已完成，显式紧凑操作待做 |
