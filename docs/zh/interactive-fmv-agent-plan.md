...
- 画布试玩分为“娱乐模式”和“实时生成”：娱乐模式从正式起点完整试玩并沿用存档/统计；实时生成提供同屏剧情树与播放器，点击任意片段（包括尚未接入主线的孤立片段）即可用全新状态从该处开始调试，不污染正式试玩记录。
- 无视频的占位片段使用中部偏上的限宽、左对齐阅读区，长正文可滚动，占位标识置于角落；选择反馈期间暂时隐藏上一片段正文与标题，不改变反馈结束后再跳转的规则。
- 实时生成顶部已预留“从此生成”入口，但当前仅展示能力位置，尚未接入从当前节点重新生成后续内容的任务链。
- 娱乐模式直接恢复已有剧情存档（从保存节点的视频开头播放，不恢复视频秒数）；实时生成提供自动播放和倍速控制，重复选择同一片段会重新播放。结局文本等视频结束后展示。
- 实时生成的横屏画面铺满播放器（按容器比例可能裁切边缘），竖屏保持完整画幅；物品锚点按实际显示方式换算。剧情树支持搜索、折叠、问题筛选和双向滚动。
