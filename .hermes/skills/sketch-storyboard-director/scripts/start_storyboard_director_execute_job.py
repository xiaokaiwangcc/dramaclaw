"""Start storyboard-director execute using storyboard_labels.jsonl by default."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start storyboard director execute job.")
    ...
    parser.add_argument("--labels-name", default="storyboard_labels.jsonl")
    ...
    if not summary_path.exists():
        raise SystemExit(
            json.dumps(
                {
                    "ok": False,
                    "reason": "missing_storyboard_labels_summary",
    ...
    if not isinstance(payload, dict):
        raise SystemExit(
            json.dumps(
                {
                    "ok": False,
                    "reason": "invalid_storyboard_labels_summary_shape",
                    "labels_jsonl": str(labels_path),
                    "summary_json": str(summary_path),
                },
                ensure_ascii=False,
    ...
    if label_count != actual_count:
        raise SystemExit(
            json.dumps(
                {
                    "ok": False,
                    "reason": "storyboard_labels_count_mismatch",
                    "labels_jsonl": str(labels_path),
                    "summary_json": str(summary_path),
    ...
    if labels_path.exists():
        ...
        if int(summary_payload.get("label_count") or 0) == 0:
            print(
                json.dumps(
                    {
                        "task_type": "storyboard_director_execute",
                        "episode": args.episode_num,
                        "labels_jsonl": str(labels_path),
                        "summary_json": str(summary_path),
                        "skipped": True,
                        "reason": "storyboard_labels.jsonl is empty; nothing to direct",
                    },
                    ensure_ascii=False,
                    indent=2,
                )
